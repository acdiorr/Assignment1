...
//All the stuff i need for the assignment!
vec3 CalcAmbient()
{
...
vec3 CalcDiffuse(vec3 worldPos, vec3 normal, vec3 camPos)
{
	vec3 viewDir  = normalize(camPos - worldPos);
	vec3 diffuseOut = vec3(0.0f, 0.0f, 0.0f);

	for(int ix = 0; ix < AmbientColAndNumLights.w && ix < MAX_LIGHTS; ix++)
	{
		// Additive lighting mode
		// Get the direction to the light in world space
		...
		diffuseOut += diffuseFactor * Lights[ix].ColorAttenuation.rgb;
	}
	return diffuseOut;

}

vec3 CalcSpecular(vec3 worldPos, vec3 normal, vec3 camPos, float shininess)
{
	vec3 specularOut = vec3(0.0f, 0.0f, 0.0f);
	vec3 viewDir  = normalize(camPos - worldPos);
	...
	for(int ix = 0; ix < AmbientColAndNumLights.w && ix < MAX_LIGHTS; ix++) {
		...
		float specPower  = pow(max(dot(normal, halfDir), 0.0), pow(256, shininess));
		// Calculate specular color
		specularOut += specPower * Lights[ix].ColorAttenuation.rgb;
