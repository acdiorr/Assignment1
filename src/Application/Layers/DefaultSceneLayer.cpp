...
void DefaultSceneLayer::_CreateScene()
{
	...
	if (loadScene && std::filesystem::exists("scene.json")) {
		...
		MeshResource::Sptr blobbyMesh = ResourceManager::CreateAsset<MeshResource>("funny.obj");
		MeshResource::Sptr catcusMesh = ResourceManager::CreateAsset<MeshResource>("cactus.obj");
		...
		Texture2D::Sptr    blobbyTex = ResourceManager::CreateAsset<Texture2D>("textures/Blobby.png");
		...
		grassTexture->SetMinFilter(MinFilter::Unknown);
		...
		Texture3D::Sptr lut3 = ResourceManager::CreateAsset<Texture3D>("luts/underworld.CUBE");
		...
		Material::Sptr boxMaterial = ResourceManager::CreateAsset<Material>(basicShader);
		{
			...
			boxMaterial->Set("u_Material.ToonLUT", toonLut);
		}
		...
		Material::Sptr grassMaterial = ResourceManager::CreateAsset<Material>(basicShader);
		{
			...
			grassMaterial->Set("u_Material.ToonLUT", toonLut);
		...
		Material::Sptr blobMaterial = ResourceManager::CreateAsset<Material>(foliageShader);
		{
			blobMaterial->Name = "Blob Mat";
			blobMaterial->Set("u_Material.Diffuse", blobbyTex);
			blobMaterial->Set("u_Material.Shininess", 0.1f);
			blobMaterial->Set("u_Material.Threshold", 0.1f);
			blobMaterial->Set("u_Material.ToonLUT", toonLut);


			blobMaterial->Set("u_WindDirection", glm::vec3(1.0f, 1.0f, 0.0f));
			blobMaterial->Set("u_WindStrength", 0.5f);
			blobMaterial->Set("u_VerticalScale", 1.0f);
			blobMaterial->Set("u_WindSpeed", 1.0f);
		...
		Material::Sptr frogMaterial = ResourceManager::CreateAsset<Material>(basicShader);
		{
			...
			frogMaterial->Set("u_Material.Shininess", 0.8f);
			frogMaterial->Set("u_Material.ToonLUT", toonLut);
		}

		Material::Sptr catcusMaterial = ResourceManager::CreateAsset<Material>(basicShader);
		{
			catcusMaterial->Name = "Catcus";
			catcusMaterial->Set("u_Material.Diffuse", catcusTex);
			catcusMaterial->Set("u_Material.Shininess", 0.1f);
			catcusMaterial->Set("u_Material.ToonLUT", toonLut);
		}
		...
		Material::Sptr foliageMaterial = ResourceManager::CreateAsset<Material>(foliageShader);
		{
			...
			foliageMaterial->Set("u_Material.ToonLUT", toonLut);
		...
		scene->Lights[0].Position = glm::vec3(10.0f, 10.0f, 100.0f);
		scene->Lights[0].Color = glm::vec3(0.49f, 0.9f, 0.29f);
		scene->Lights[0].Range = 650.0f;
		scene->Lights[0].intensity = 10000.f;

		/*
		scene->Lights[0].Position = glm::vec3(1.0f, 0.0f, 3.0f);
		scene->Lights[0].Color = glm::vec3(0.49f, 0.9f, 0.29f);
		...
		*/
		GameObject::Sptr blob = scene->CreateGameObject("Funny Man");
		{
			// Set position in the scene
			blob->SetPostion(glm::vec3(1.5f, 0.0f, 0.5f));
			blob->SetRotation(glm::vec3(90.0f, 0.0f, 0.0f));

			// Create and attach a renderer for the monkey
			RenderComponent::Sptr renderer = blob->Add<RenderComponent>();
			renderer->SetMesh(blobbyMesh);
			renderer->SetMaterial(blobMaterial);

			demoBase->AddChild(blob);

		}


		GameObject::Sptr blob2 = scene->CreateGameObject("Funny Man 2");
		{
			// Set position in the scene
			blob2->SetPostion(glm::vec3(7.5f, 0.0f, 0.5f));
			blob2->SetRotation(glm::vec3(90.0f, 0.0f, 0.0f));

			// Create and attach a renderer for the monkey
			RenderComponent::Sptr renderer = blob2->Add<RenderComponent>();
			renderer->SetMesh(blobbyMesh);
			renderer->SetMaterial(blobMaterial);

			demoBase->AddChild(blob2);
		...
		{
			// Set position in the scene
			frog->SetPostion(glm::vec3(5.f, 1.0f, 0.0f));
			...
			demoBase->AddChild(frog);

		}

		GameObject::Sptr catcus = scene->CreateGameObject("Catcus");
		{
			// Set position in the scene
			catcus->SetPostion(glm::vec3(3.f, 10.0f, -2.2f));
			catcus->SetRotation(glm::vec3(90.0f, 0.0f, 0.0f));

			catcus->SetScale(glm::vec3(5.0f, 5.0f, 5.0f));
